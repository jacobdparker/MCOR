...
;set dat to choose different columns to compare
  ...
  IF keyword_set(ex) then begin
     gv = where((valid EQ 1) AND (finite(modout)) AND (finite(y)),nvalid)
  ...
  IF nvalid GT 1 THEN BEGIN 
     IF keyword_set(ex) then sq = total((modout - y)^2)/total(y^2) else sq = total( (modout[dat[0],gv] - y[dat[1],gv])^2 )

;attempt to use a zero lag cc instead of simple least squares
     print,sq
...
FUNCTION mcor_fit,x,y,param,model,valid,dat=dat,_EXTRA=ex,modout=modout
;default is to call model with x,param, compare output[0,*] to y[0,*]
;set dat to choose different columns to compare

  modout = call_FUNCTION(model,x,param,_EXTRA=ex)
  loglike = c_correlate(modout(0,*),y(0,*),0)
  loglike = alog(loglike)
  return,loglike
...
;reset values
    ...
    IF (random_start EQ 1) AND (c GT 0) THEN value[change] = min[change] + (range[change] * randomu(seed,nchange))
...
PRO MCOR_MCMC,sav
;An attempt to fit MOSES CC function using MCMC
;To run, start IDL
;type .compile mcmc.pro
;then type mcmc_example
;type retall once done to break out of debug mode

;initialize problem, common block required for mcor_forward_mcmc
...
mcor = reform(mcor,1,n_elements(mcor))

y = mcor

;The input parameter file for mcmc
  param = { name :       ['heii',  'ar',   'qs',  'qs_eis',  'prom',  'chole',  'flare'], $
            value :      [0,1,1,0,0,0,0], $
            max :        [100,10,10,10,10,10,10], $
            min :        [0,0,0,0,0,0,0], $
            knob :       [0,0,0,0,0,0,0], $
            changeable : [1,1,1,1,1,1,1] }

;Run through mcor_forward_mcmc (not needed)
;tcor = mcor_forward_mcmc(eit_ch,param.value)



model='mcor_forward_mcmc'
likelihood='mcmc_likelihood' 


;Call MCMC
  mcmc,eit_ch,y,param,outputll=outputll,outputvalue=outputvalue,outputy=y,model=model,likelihood=likelihood,/random_start,numchains=50

;Plot outputs
  
 
  print,'Best:  ',float(outputvalue[*,0])
  print,'Fit Quality= ',total((y-mcor)^2)

save,eit_ch,outputvalue,outputll,y,filename=sav
toc
