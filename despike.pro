...
;CALLING SEQUENCE:
;  result = iris_prep_despike(data [, sigmas=sigmas] [, Niter=Niter])
;INPUT PARAMETERS:
...
;  2013-Dec-10  P. Boerner copied from CK's DESPIK.PRO for placement in the IRIS
;				SSW library

function despike, data, sigmas=sigmas, Niter=Niter, kernel=kernel, min_std=min_std, $
   silent=silent, goodmap=goodmap, mode=mode

if not keyword_set(silent) then begin 
   print, systime()+' IRIS_PREP_DESPIKE started on array of ', n_elements(data),' elements.'
   print, 'Step (1): Iteratively identifying bad pixels.'
endif

;Deal with negative image pixels (probably from eit_prep)
negative = where(data lt 0)
data(negative) = 1e-6
...
neighborhood_mean = convol(goodmap*data, k2, /edge_truncate) / convol(goodmap,      k2, /edge_truncate)
...
if not keyword_set(silent) then print,systime()+' IRIS_PREP_DESPIKE finished, ',t_end-t_begin,' sec elapsed.'
return, result
